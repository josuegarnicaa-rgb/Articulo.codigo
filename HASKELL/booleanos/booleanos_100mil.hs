...
main = do
  ...
  contenido <- readFile "booleanos_100mil.txt"
  
  -- 2. Convertimos el texto en una lista de enteros
  -- 'words' separa el texto por espacios o saltos de línea
  -- 'map read' convierte cada fragmento de texto en un número Int
  let listaBooleanos = map read (words contenido) :: [Bool]
  
  putStrLn "Iniciando ordenamiento..."
  
  -- 3. Forzamos la evaluación del algoritmo para medir el tiempo real
  listaOrdenada <- evaluate (quicksort listaBooleanos)
